...
for col in categorical_feature:
    copytrain2[col].value_counts().plot(kind='bar')
    plt.title(col)
    plt.show()
...
for col in categorical_feature:
    pretrain2[col] = copytrain2[col]

pretrain2['Year'] = copytrain2['발생일시'].apply(make_year)
pretrain2['Month'] = copytrain2['발생일시'].apply(make_month)
pretrain2['Day'] = copytrain2['발생일시'].apply(make_day)
...
pretrain2['EPDO'] = pretrain2.apply(
    lambda x: x['D_acci']*12 + x['S_acci']*5 + x['C_acci']*3 + x['I_acci'], axis=1)

print(pretrain2)


for col in ['Year', 'Month', 'Day', 'Hour', 'EPDO']:
    pretrain2[col].value_counts().sort_index(ascending=True).plot(kind='bar')
    plt.title(col)
    plt.show()
...
pretrain2['자전거도로_여부'].value_counts(dropna=False)


# pretrain2[pretrain2['자전거도로_여부'] == "예"].to_csv(
#     "자전거도로Yes데이터.csv", encoding="EUC-KR")
############################
...
# pretrain2['acci_case_D'] = copytrain2['사고유형'].apply(
#     make_acci_D)   # 추후 interaction을 통해 파생변수 생성
...
print(pretrain2)
# 사고유형 세부사항 : 13가지 항목, 기타항목이 1/3을 차지함
print(pretrain2['acci_case_D'].value_counts())
# 법규위반 세부사항 : 11가지 항목, 사고가 일어나면 무조건 법규위반으로 처리 -> 안전운전불이행, 안전거리 미확보 등등으로 빠진 것 같음
print(copytrain2['법규위반'].value_counts())
# 노면상태 세부사항 : 7가지 항목, 침수&해빙은 데이터가 없다시피 한 수준
print(pretrain2['road_stat_D'].value_counts())
print(copytrain2['기상상태'].value_counts())      # 기상상태 세부사항 : 6가지 항목
# 도로형태 세부사항 : 10가지 항목, 기타가 절반이상을 차지
print(pretrain2['road_form'].value_counts())
# 가해운전자 연령 : 미분류 데이터 140개(-1로 임의부여), 필요 시 범위단위로 나눌 필요성 존재
print(pretrain2['at_age'].value_counts())
print(pretrain2['acci_case_D'].value_counts())
...
train1.to_csv(save_path + "train_FeatureSelection.csv",
...
pretrain2.to_csv(save_path + "자전거,PM,이륜차사고목록_전처리(2017-2020).csv",
                 encoding="EUC-KR", index=False)
